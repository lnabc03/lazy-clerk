...
**lazy-clerk** 是一个面向实习医生的考勤自动签到工具。每天定时（上午约 6:53–6:58、下午约 13:00–13:05，具体见各形态）自动登录医院统一登录平台，进入实习考勤系统，完成当前时间段的签到。
...
```
触发（cron 6:53 / 13:00 + 0–300s 随机抖动，时区 Asia/Shanghai）
  │
...
| 时段 | 主触发   | 重试     | 停止时间                   |
| -- | ----- | ------ | ---------------------- |
| 上午 | 6:53  | 每 5 分钟 | 7:28（窗口 8:00 关闭前留缓冲）   |
| 下午 | 13:00 | 每 5 分钟 | 14:23（窗口 14:30 关闭前留缓冲） |
...
**个人页（/me）**：顶部昵称 + 退出按钮；状态卡片（启用状态、下次执行时间 = 下一个 6:53 或 13:00）；"立即签到"按钮；SendKey 修改表单；近 7 天签到记录表（日期 / 时段 / 结果 / 信息）。**任何页面都不展示密码，SendKey 掩码展示。**
