...
**lazy-clerk** 是一个面向实习医生的考勤自动签到工具。每天定时（上午 6:58、下午 13:58）自动登录医院统一登录平台，进入实习考勤系统，完成当前时间段的签到。
...
```
触发（cron 6:58 / 13:58，时区 Asia/Shanghai）
  │
...
| 时段 | 主触发   | 重试     | 停止时间                   |
| -- | ----- | ------ | ---------------------- |
| 上午 | 6:58  | 每 5 分钟 | 7:28（窗口 8:00 关闭前留缓冲）   |
| 下午 | 13:58 | 每 5 分钟 | 14:23（窗口 14:30 关闭前留缓冲） |

- 重试**整体重跑主流程**（会话分钟级过期，每次必须重新登录）
- 同一时段同一账号：成功即停止；超过停止时间仍失败 → 推送最终告警
- 注意：若 6:58 失败、7:03 重试成功，用户会在 7:00 收到一次系统提醒——无害，属预期现象
...
**个人页（/me）**：顶部昵称 + 退出按钮；状态卡片（启用状态、下次执行时间 = 下一个 6:58 或 13:58）；"立即签到"按钮；SendKey 修改表单；近 7 天签到记录表（日期 / 时段 / 结果 / 信息）。**任何页面都不展示密码，SendKey 掩码展示。**
...
| `lazy-clerk.exe install` | 注册 Windows 计划任务：每天 6:58 / 13:58 各一条 |
